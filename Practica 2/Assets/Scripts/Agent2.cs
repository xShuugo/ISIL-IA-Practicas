...
{   
    ...
    void Update()
    {
        ...
        if(transform.position.x <= (originPos.x - AreaSquare) || transform.position.x >= (originPos.x + AreaSquare) ||
        transform.position.y <= (originPos.y - AreaSquare) || transform.position.y >= (originPos.y + AreaSquare)){
            velocity += Seek(originPos);
